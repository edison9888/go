...
- (void)authorizeWithTencentAppAuthInSafari:(BOOL) bInSafari{
	...
	if (!didOpenOtherApp)
	{
        ...
        TencentLoginViewController *tencentLoginViewController = [[TencentLoginViewController alloc] init];
        [tencentLoginViewController initWithTencentLoginView:_loginDialog];
        UINavigationController *navController = [[UINavigationController alloc] initWithRootViewController:tencentLoginViewController];
...
- (void)logout:(id<TencentSessionDelegate>)delegate
{
    ...
    if ([delegate respondsToSelector:@selector(tencentDidLogout)])
    {
        [delegate tencentDidLogout];
    }
}
